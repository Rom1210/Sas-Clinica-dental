...
const PatientProfile = ({ patient: propPatient, onBack: propOnBack }) => {
  ...
  // Tab/Section Scrolling Effect
  useEffect(() => {
    if (location.state?.scrollSection === 'odontogram' && activeTab === 'General') {
      const scrollTimer = setTimeout(() => {
        const element = document.getElementById('odontogram-section');
        if (element) {
          element.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }, 300); // Wait for tab animation/render
    ...
    const cons = allConsultations
      ...
      .map(c => {
        ...
        return {
          ...
          notes: c.reason || '', amount: c.amount, icon: <Activity />
        };
      });
  ...
  if (!patient) return (
    <div className="flex flex-col gap-8 p-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <Skeleton height="200px" borderRadius="2rem" />
         <Skeleton height="200px" borderRadius="2rem" />
         <Skeleton height="200px" borderRadius="2rem" />
      </div>
    </div>
  ...
  const tabs = ['General', 'Perfil', 'Galería', 'Planes', 'Historia médica', 'Historia de pago'];
